...
function Nav() {
 ...
 dispatch(getTotal())
  ...
  useEffect(() => {
    if (mobileVisable && mobileNavRef.current) {
    ...
    <div className=''>
      <div className={`upper-nav px-mb_side lg:px-side py-top flex flex-col lg:flex-row lg:justify-between items-center`}>
        ...
        <div className="upper-nav-right flex flex-row-reverse justify-center lg:justify-normal lg:flex-row flex-wrap-reverse items-center lg:gap-4 gap-4">
          <div className="searchbox flex justify-center items-center w-full lg:w-fit overflow-hidden rounded-md">
            <input type="text"
      ...
      <div className="lower-nav bg-secondary px-mb_side lg:px-side">
        <div className="navigation flex text-white">
          <div className={`py-4 px-6  font-medium relative hover:text-tertiary cursor-pointer pl-0 flex items-center gap-3 my-auto`} onClick={() => setMobileVisable(!mobileVisable)}>
            <GiHamburgerMenu />
            ...
            {mobileVisable && (
              <div className="w-screen for-mobile lg:hidden overflow-hidden z-50 flex flex-col absolute top-full -left-6 bg-secondary rounded-sm" ref={mobileNavRef}>
                ...
                <NavLink className="py-4 px-6 font-medium hover:text-tertiary cursor-pointer" to='/learn' onClick={() => setMobileVisable(false)}>Learn</NavLink>
                {user && user.userType === 'admin' && (
                  <NavLink to={'/admin?tab=product'} className="cart flex justify-center items-center">
                    <button className='bg-secondary font-bold px-3 py-2 text-white' onClick={() => setMobileVisable(false)}>Manage</button>
                  </NavLink>
                )}
              </div>
            )}
          </div>
          <div className={`${nav_item_style} relative`} onMouseEnter={() => setVisable(true)} onMouseLeave={() => setVisable(false)}>
            <NavLink to='/products'>
            ...
            {visable && (
              <div ref={subMenuRef} className='overflow-hidden products-sub-menu z-10 flex flex-col absolute top-full bg-secondary rounded-sm'>
